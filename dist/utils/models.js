...
let cachedModels = loadPersistedModels();
...
function hasModelsChanged(existing, fresh) {
...
export async function fetchUpstreamModels(apiKey, ccVersion) {
        ...
        try {
            ...
            if (res.ok) {
                ...
                if (Array.isArray(data))
                    ...
                    rawList = data.models;
                if (rawList.length > 0) {
                    const freshModels = rawList.map(item => {
...
export function resolveModelName(requestedModel) {
    ...
    const endsWithMatch = available.find(m => m.id.toLowerCase() === lowerClean ||
