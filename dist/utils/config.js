...
function getProjectRootDir() {
    if (process.execPath.toLowerCase().includes('commandcode-proxy-v3') || process.pkg) {
...
export function getGatewayRunning() {
    return gatewayRunning;
}
export function setGatewayRunning(state) {
    gatewayRunning = state;
}
export function getCliAuthKey() {
    try {
...
export function syncEnvFile(accounts, activeApiKey) {
    try {
        const envLines = [
...
export function getActiveApiKey() {
    ...
    if (config.accounts.length > 0) {
        ...
        if (config.accounts[0].apiKey)
            return config.accounts[0].apiKey;
    }
...
export async function fetchLiveUsageStats(apiKey, ccApiBase, ccVersion) {
    const headers = {
    ...
    const results = { whoami: null, credits: null, summary: null };
    try {
        const resWho = await fetch(`${ccApiBase}/whoami`, { headers });
        if (resWho.ok)
            results.whoami = await resWho.json();
    }
    catch { }
    try {
        const resCred = await fetch(`${ccApiBase}/credits`, { headers });
        if (resCred.ok)
            results.credits = await resCred.json();
    }
    catch { }
    try {
        const resSum = await fetch(`${ccApiBase}/usage/summary`, { headers });
        if (resSum.ok)
            results.summary = await resSum.json();
    }
    catch { }
    return results;
}
export async function checkAndRotateAccountsOnQuota() {
    const config = loadConfig();
    if (config.rotationMode !== 'auto-quota')
        return;
...
export function openBrowser(url) {
    ...
    exec(`${startCmd} ${url}`, (err) => {
        if (err)
            logger.warn(`[BROWSER] Could not auto-open browser URL: ${err.message}`);
    });
}
